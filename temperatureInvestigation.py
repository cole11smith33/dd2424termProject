    ...
    with torch.no_grad():
        for _ in range(length):
            ...
            if nucleus_p is not None:
                ...
                nucleus_mask = (cumulative_probs - sorted_probs) < nucleus_p
                sorted_probs[~nucleus_mask] = 0.0

                sorted_probs = sorted_probs / sorted_probs.sum()

                sampled_pos = torch.multinomial(sorted_probs, num_samples=1)
                next_index  = sorted_indices[sampled_pos].item()
            else:
...
print("\nTemperature & Nucleus Sampling Investigation")

temperatures = [0.5, 1.0, 1.5]
nucleus_ps   = [None, 0.9, 0.95]

for temp in temperatures:
    for p in nucleus_ps:
        label = f"temperature={temp}, nucleus_p={'off' if p is None else p}"
        print(f"\n{'─'*60}")
        print(label)
        print('─'*60)
        print(generate_text(
            best_model, "T", char_to_ind, ind_to_char, vocab_size,
            length=300, temperature=temp, nucleus_p=p, device=device
        ))